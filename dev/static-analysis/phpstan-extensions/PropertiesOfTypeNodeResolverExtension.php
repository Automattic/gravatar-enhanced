...
/**
 * Extension for PHPStan to support properties-of custom type.
 */
class PropertiesOfTypeNodeResolverExtension implements TypeNodeResolverExtension, TypeNodeResolverAwareExtension {
	...
	public function resolve( $type_node, $name_scope ): ?Type {
		...
		if ( $type_name->name !== 'properties-of' ) {
			return null;
		}
		...
		foreach ( $argument_types as $argument_type ) {
			if ( $argument_type instanceof ObjectType ) {
				...
				foreach ( $php_reflection_class->getProperties() as $property ) {
					if ( $reflection_class->hasProperty( $property->name ) ) {
						...
						$merged_type_builder->setOffsetValueType(
							new ConstantStringType( $property->name ),
							$readable_type,
						);
					}
				}
				...
				foreach ( $argument_type->getProperties() as $property_name => $property_type ) {
					$merged_type_builder->setOffsetValueType(
						new ConstantStringType( $property_name ),
						$property_type,
					);
				}
			} else {
				throw new ShouldNotHappenException( 'properties-of only supports object types or phpstan type that resolve to objects ' );
			}
		}
